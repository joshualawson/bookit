...
var tabs_1 = __webpack_require__(/*! ./tabs/tabs */ "./tabs/tabs.ts");
...
var tab_1 = __webpack_require__(/*! ./tabs/tab */ "./tabs/tab.ts");
var Bookit = /** @class */ (function () {
    function Bookit() {
        var _this = this;
        this.tabs = new tabs_1.Tabs();
        webextension_polyfill_ts_1.browser.pageAction.onClicked.addListener(function () {
            _this.toggleBookit();
            webextension_polyfill_ts_1.browser.storage.sync.get().then(function (data) {
                console.log(data);
            });
        });
        webextension_polyfill_ts_1.browser.runtime.onMessage.addListener(function (msg, sender) {
            var tab = _this.tabs.findTab(sender.tab.id);
    ...
    Bookit.prototype.toggleBookit = function () {
        var _this = this;
        webextension_polyfill_ts_1.browser.tabs.query({ active: true, windowId: webextension_polyfill_ts_1.browser.windows.WINDOW_ID_CURRENT })
            .then(function (tabs) { return webextension_polyfill_ts_1.browser.tabs.get(tabs[0].id); })
            .then(function (tab) {
            var t = _this.tabs.findTab(tab.id);
            if (t == null || !t.getActive()) {
                if (t == null) {
                    webextension_polyfill_ts_1.browser.tabs.sendMessage(tab.id, { "event": "REQUEST_SCROLL_POS" }).then(function (resp) {
                        t = new tab_1.Tab(tab.id, true, tab.url, tab.title, resp.scroll_pos);
                        _this.tabs.appendTab(t);
                        t.save();
                    });
                }
                else {
            ...
            _this.tabs.deactivateTab(tab.id);
        });
    };
    return Bookit;
}());
new Bookit();
webextension_polyfill_ts_1.browser.storage.sync.clear();
...
var Tab = /** @class */ (function () {
    function Tab(tabId, active, url, title, scrollPos, id) {
        if (id == null) {
            this.id = uuid_1.v4();
        }
        this.tabId = tabId;
    ...
    Tab.prototype.getScrollPos = function () {
        return this.scrollPos;
    };
    Tab.prototype.setScrollPos = function (scrollPos) {
        this.scrollPos = scrollPos;
    };
    Tab.prototype.save = function () {
        var _this = this;
        console.log("saving tab -- url: " + this.url + " title: " + this.title + " scrollPos: " + this.scrollPos);
        webextension_polyfill_ts_1.browser.storage.sync.get().then(function (data) {
            ...
            webextension_polyfill_ts_1.browser.storage.sync.set(data);
        });
    };
    return Tab;
}());
...
var Tabs = /** @class */ (function () {
    function Tabs() {
        ...
        webextension_polyfill_ts_1.browser.tabs.onUpdated.addListener(function (tabId, tabInfo) {
            _this.updateActiveTab(tabId, tabInfo);
        });
        webextension_polyfill_ts_1.browser.tabs.onRemoved.addListener(function (tabId) {
            _this.closeActiveTab(tabId);
        });
    }
    Tabs.prototype.appendTab = function (tab) {
        this.collection.push(tab);
    };
    Tabs.prototype.findTab = function (tabId) {
        return this.collection.find(function (elm) {
            return elm.getTabId() === tabId;
        });
    };
    Tabs.prototype.deteleTab = function (tabId) {
        var tab = this.findTab(tabId);
        if (tab) {
            var index = this.collection.indexOf(tab);
            if (index !== -1) {
                this.collection.splice(index, 1);
            }
        }
    };
    Tabs.prototype.updateActiveTab = function (tabId, tabInfo) {
        var tab = this.findTab(tabId);
        if (tab === null || tab === void 0 ? void 0 : tab.getActive()) {
            ...
            webextension_polyfill_ts_1.browser.tabs.sendMessage(tab.getTabId(), { "event": "REQUEST_SCROLL_POS" }).then(function (resp) {
                tab.setScrollPos(resp.scroll_pos);
            });
            return;
        }
    };
    Tabs.prototype.closeActiveTab = function (tabId) {
        var tab = this.findTab(tabId);
        if (tab != null && tab.getActive()) {
            tab.save();
        }
    ...
    Tabs.prototype.activateTab = function (tabId) {
        var tab = this.findTab(tabId);
        tab === null || tab === void 0 ? void 0 : tab.setActive(true);
    };
    Tabs.prototype.deactivateTab = function (tabId) {
        var tab = this.findTab(tabId);
        tab === null || tab === void 0 ? void 0 : tab.setActive(false);
    };
    return Tabs;
}());
exports.Tabs = Tabs;
